...
const float gravitatesKonstante = 0.5; // weak atomic force
const float antiGravitatesKonstante = 0; // strong atomic force
const float centraGravitatesKonstante = 10;
const float centraRadiuss = 10;
const float pretestibasKonstante = 0.000001;
...
particle imageDefParticle = { 0, 0, 0, 0, 1, 1, 2, 1 };
...
{
    if (desiredMagnitude == 0) { return nullVector2f; }
...
int physicsUpdate(double deltaTime)
{
    for (int i = 0; i < partikles; i++)
    {
        if (pretestibaEksiste) // apply drag
        {
            ...
            vector2f dragForce = coefficientVektor(dragForceDirection, pretestibasKonstante * speedSquared);
            maksimilianaKungs[i].vx += dragForce.x;
            maksimilianaKungs[i].vy += dragForce.y;
        }
        // apply acceleration;
